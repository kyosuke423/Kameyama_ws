      ...
      mTimeStamp(timeStamp), mK(K.clone()), mDistCoef(distCoef.clone()), mbf(bf), mThDepth(thDepth), mpReferenceKF(static_cast<KeyFrame *>(NULL))
{
    ...
    if (whether_dynamic_object)
    {
        ...
        objmask_img = cv::imread(pred_mask_img_name, cv::IMREAD_UNCHANGED); // uint8  sometimes read image might take long time....  //11.24new CV_LOAD_IMAGE_UNCHANGEDをcv::IMREAD_UNCHANGEDに
