...
#include <unistd.h> //usleepの解決

using namespace std;
/*
//2024/8/6 kameyama
//CSVファイルに保存する関数
void writeDimensionsToCSV(const std::string& filename, const std::vector<std::vector<double>>& dimensions) {
    std::ofstream file(filename);

    if (!file.is_open()) {
        std::cerr << "ファイルを開くのに失敗しました: " << filename << std::endl;
        return;
    }

    file << "ObjectID,CuboidID,Height,Width,Depth\n";

    for (const auto& dimension : dimensions) {
        file << std::fixed << std::setprecision(2)
             << dimension[0] << ','  // ObjectID
             << dimension[1] << ','  // CuboidID
             << dimension[2] << ','  // Height
             << dimension[3] << ','  // Width
             << dimension[4] << '\n'; // Depth
    }
																	 ...
																	 mpFrameDrawer(pFrameDrawer), mpMapDrawer(pMapDrawer), mpMap(pMap), mnLastRelocFrameId(0)
{
	...
	// n.param<double>("init_z", init_z, 0.63);
	n.param<double>("init_z", init_z, 0);
	...
	cout << "- y: " << init_y << endl;
	cout << "- z: " << init_z << endl;
	cout << "- qx: " << init_qx << endl;
	cout << "- qy: " << init_qy << endl;
	cout << "- qz: " << init_qz << endl;
	cout << "- qw: " << init_qw << endl;
...
cv::Mat Tracking::GrabImageRGBD(const cv::Mat &imRGB, const cv::Mat &imD, const double &timestamp)
{
	...
	// 	imDepth.convertTo(imDepth, CV_32F, mDepthMapFactor); //12.8new
...
cv::Mat Tracking::GrabImageMonocular(const cv::Mat &im, const double &timestamp, int msg_seq_id)
{
	...
	// printf("GrabImageMonocular!"); //11.17debug
	return mCurrentFrame.mTcw.clone();
}
...
void Tracking::Track()
{
	...
	if (mState == NOT_INITIALIZED) // initialization 初期化
	{
		...
		else
		{
			...
			if (mCurrentFrame.mnId == 0)
			{
				if (mono_firstframe_truth_depth_init)
				{
					...
					StereoInitialization(); // if first frame has truth depth, we can initialize simiar to stereo/rgbd. create keyframe for it.　深度がある場合
				}
			...
			if (!special_initialization)
				MonocularInitialization(); // usually for monocular, need to wait for several frames, with enough parallax　単眼用、深度なし
		}
	...
	else
	{
		...
		// Initial camera pose estimation using motion model or relocalization (if tracking is lost) 初期カメラポーズ推定
		if (!mbOnlyTracking)
		{
			// Local Mapping is activated. This is the normal behaviour, unless explicitly activate the "only tracking" mode.　ローカルマッピング有効
			if (mState == OK)
			{
				// Local Mapping might have changed some MapPoints tracked in last frame　ローカルマッピングは最後のフレームのマップポイントを変更した可能性がある
				CheckReplacedInLastFrame();

				if (mVelocity.empty() || mCurrentFrame.mnId < mnLastRelocFrameId + 2) // at initialization or relocalization stage　初期化or再局在段階
				{
		...
		else // Only Tracking: Local Mapping is deactivated, usually not happening.
		{
			if (mState == LOST)
			{
				ROS_ERROR_STREAM("Last frame tracking bad, start relocalization!!"); //11.18debug
				bOK = Relocalization();
			}
			else
			{
		...
		else
		{
			...
			mState = LOST; //11.18debug これがLOSTの原因だった
		}
		...
		if (bOK)
		{
			...
			if (!mLastFrame.mTcw.empty())
			{
				...
				// std::cout << "camera pose : " << LastTwc << std::endl; //2.11new
			}
			else
				mVelocity = cv::Mat();
...
void Tracking::StereoInitialization()
{
	...
	if (mCurrentFrame.N > 500)
	{
		...
		if (build_worldframe_on_ground) // transform initial pose and map to ground frame 初期ポーズを変換し、地面フレームにマップします
...
{
	// printf("CheckReplacedInLastFrame..."); //11.17debug
...
bool Tracking::TrackWithMotionModel()
{
	...
	if (nmatches < 20)
	{
		// printf("uses a wider window search...\n");  //11.17debug
	...
	if (whether_detect_object)
	{
		if (use_dynamic_klt_features)
			...
			// printf("use detect object matcher\n");  //11.17debug
	}
	...
	// printf("TrackWithMotionModel!"); //11.17debug
	return nmatchesMap >= 10;
}
...
void Tracking::DetectCuboid(KeyFrame *pKF)
{
	...
	else
	{
		...
		//ORB_SLAM2::ImageGrabber*getdata(all_lines_raw, raw_all_obj2d_bbox);

		//line start 12.14new
		line_lbd_detect* line_lbd_ptr = new line_lbd_detect(1,2.0); 
		...
		line_lbd_ptr->detect_raw_lines(pKF->raw_img,keylines_raw);
		...
		//line finish 12.14new

		// remove some 2d boxes too close to boundary.
		int boundary_threshold = 20;
		int img_width = pKF->raw_img.cols;
		int img_height = pKF->raw_img.rows;
		std::vector<int> good_object_ids;
		std::vector<int> bad_object_ids; //2.2new yoloチェック用

		for (int i = 0; i < raw_all_obj2d_bbox.rows(); i++)
			if ((raw_all_obj2d_bbox(i, 0) > boundary_threshold) && (raw_all_obj2d_bbox(i, 1) > boundary_threshold) && (raw_all_obj2d_bbox(i, 0) + raw_all_obj2d_bbox(i, 2) < img_width - boundary_threshold) 
			&& (raw_all_obj2d_bbox(i, 1) + raw_all_obj2d_bbox(i, 3) < img_height - boundary_threshold) && (raw_all_obj2d_bbox(i, 2) >= 10))
				good_object_ids.push_back(i);
			else
				std::cout << "bad object : " << raw_all_obj2d_bbox.row(i) << std::endl;

		Eigen::MatrixXd all_obj2d_bbox_infov_mat(good_object_ids.size(), 8);
		...
		ROS_INFO("yolodata");
		// for(size_t i = 0; i < good_object_ids.size(); i++){
		// 	std::cout << all_obj2d_bbox_infov_mat(i, 5) << ", ";
		// }
		// std::cout << std::endl;
		std::cout << all_obj2d_bbox_infov_mat << std::endl;
		// printf("detect cuboid keisan...\n"); //11.17debug
		...
		// printf("keisan finish!\n"); //11.17debug

		/*
		//2024/8/6 kameyama
		std::vector<std::vector<double>> dimensions;
		// 直方体の内容を表示
        for (int object_id = 0; object_id < all_obj_cubes.size(); object_id++) {
            std::cout << "Object ID: " << object_id << std::endl;

            for (int cube_id = 0; cube_id < all_obj_cubes[object_id].size(); cube_id++) {
                std::cout << "Cuboid ID: " << cube_id << std::endl;
                std::cout << *(all_obj_cubes[object_id][cube_id]) << std::endl; // cuboid の情報を表示

                // 追加: 直方体の寸法を表示
                double height = all_obj_cubes[object_id][cube_id]->scale(2);
                double width = all_obj_cubes[object_id][cube_id]->scale(0);
                double depth = all_obj_cubes[object_id][cube_id]->scale(1);

                // cuboidオブジェクトにサイズ情報をセット　試す
                all_obj_cubes[object_id][cube_id]->scale << width, depth, height;
				...
				// 寸法をCSVに書き込む
                //writeDimensionsToCSV("cuboid_dimensions.csv", dimensions);
            }
        }
		*/
	}
//オフラインで使える↑all_obj2d_bbox_infov_mat
	...
	{
		// std::cout << "detect object num   " << all_obj_cubes.size() << std::endl;
		std::cout << "object num   " << all_obj_cubes[ii].size() << std::endl;
		if (all_obj_cubes[ii].size() > 0) // if has detected 3d Cuboid
		{
			...
			//10.24new
			newcuboid->classnameid = raw_cuboid->classname;
			newcuboid->yolo_object_id = raw_cuboid->idname;
			...
			if (use_truth_trackid)
				...
				// printf("use_truth_trackid"); //11.17debug
			...
			if (1)
			{
				...
				// printf("obj_cam_dist"); //11.17debug
			}
			else
				newcuboid->meas_quality = 1.0;
				// printf("meas_quality = 1 "); //11.17debug
			if (all_box_confidence[ii] > 0)
				newcuboid->meas_quality *= all_box_confidence[ii]; // or =
				// printf("meas_quality *= all_box_confidence"); //11.17debug
	...
	// printf("DetectCuboid!"); //11.17debug
}
...
void Tracking::AssociateCuboids(KeyFrame *pKF)
{
	...
	int largest_shared_num_points_thres = 3; //1.29new もともとは10
	if (mono_allframe_Obj_depth_init)
	...
	for (size_t i = 0; i < LocalObjectsCandidates.size(); i++)
	{
		...
		else // if found, then update observation.
		{
			...
			ROS_WARN_STREAM("object merge!!!!!!!!!!!"); //11.17debug
		}
	}
	...
	if (remove_object_outlier)
	{
		...
		for (size_t i = 0; i < all_objects.size(); i++)
		{
			...
			//1.18new
			Eigen::MatrixXd cube_corners_check;
			cube_corners_check = pMObject->GetWorldPos().compute3D_BoxCorner();
			vector<float> box_x;
			vector<float> box_y;
			vector<float> box_z;
			for(int b = 0; b < 8; b++)
			{
				box_x.push_back(cube_corners_check(0, b));
				box_y.push_back(cube_corners_check(1, b));
				box_z.push_back(cube_corners_check(2, b));
			}
			...
			if (kidou)
				if (!kidou->isBad())
				{
					...
					// std::cout << "camera: " << kidou_check.at<float>(0) << "," << kidou_check.at<float>(1) << "," << kidou_check.at<float>(2) << std::endl;
					// std::cout << "box: " << x_min << "~" << x_max << "," << y_min << "~" << y_max << "," << z_min << "~" << z_max << std::endl;
					//glVertex3f(currOw.at<float>(0), currOw.at<float>(1), currOw.at<float>(2));
					if((x_min < kidou_check.at<float>(0)) && (kidou_check.at<float>(0) < x_max) && (y_min < kidou_check.at<float>(1)) && (kidou_check.at<float>(1) < y_max) && (z_min < kidou_check.at<float>(2)) && (kidou_check.at<float>(2) < z_max))
					{
						ROS_WARN_STREAM("buttai error!!!!!!!!!!!");
						std::cout << "camera: " << kidou_check.at<float>(0) << "," << kidou_check.at<float>(1) << "," << kidou_check.at<float>(2) << std::endl;
						std::cout << "box: " << x_min << "~" << x_max << "," << y_min << "~" << y_max << "," << z_min << "~" << z_max << std::endl; 
						pMObject->SetBadFlag();
						if (use_truth_trackid)
	...
	printf("AssociateCuboids!"); //11.17debug
}
...
void Tracking::CreateNewKeyFrame()
{
	...
	mnLastKeyFrameId = mCurrentFrame.mnId;
	mpLastKeyFrame = pKF;
	// printf("CreateNewKeyFrame!"); //11.17debug
}
...
void Tracking::SearchLocalPoints()
{
	...
	// printf("SearchLocalPoints!"); //11.17debug
}
...
void Tracking::UpdateLocalMap()
{
	...
	UpdateLocalPoints();
	// printf("UpdateLocalMap!"); //11.17debug
}
...
void Tracking::UpdateLocalPoints()
{
	...
	// printf("UpdateLocalPoints"); //11.17debug
}
...
void Tracking::UpdateLocalKeyFrames()
{
	...
	// printf("UpdateLocalKeyFrames!"); //11.17debug
}
