...
FrameDrawer::FrameDrawer(Map *pMap) : mpMap(pMap)
{
	...
	for(int i = 1; i < 6; i++)
	{
		...
		for(int j = 0; j < 6; j++)
		{
			...
			{
				color_id3_f = 51 + 51 * k;
				color_ids_frame.push_back(cv::Scalar(color_id3_f, color_id2_f, color_id1_f));
			}
		}
	}
}
...
cv::Mat FrameDrawer::DrawFrame()
{
    ...
    if (im.channels() < 3) //this should be always true
        cvtColor(im, im, cv::COLOR_GRAY2BGR); //11.24new CV_GRAY2BGRをcv::COLOR_GRAY2BGRに
    ...
    {                          // better to write some warning that if it is keyframe or not, because I only detect cuboid for keyframes.
        ...
        {
            cv::Scalar box_colors_frame;
            box_colors_frame = color_ids_frame[classname_frame[i]];
            // cv::rectangle(im, bbox_2ds[i], box_colors[i % box_colors.size()], 2); // 2d bounding box. 原文
            cv::rectangle(im, bbox_2ds[i], box_colors_frame, 2); //
            ...
            if ((pTracker->mCurrentFrame.mnId - pTracker->mCurrentFrame.mpReferenceKF->mnFrameId) < 1) // if current frame is a keyframe
            {
                if (whether_detect_object)
                {
                    for (const MapObject *object : pTracker->mCurrentFrame.mpReferenceKF->local_cuboids)
                    {
                        ...
                        classname_frame.push_back(object->classnameid); //2.4new
