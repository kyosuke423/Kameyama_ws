...
#include <opencv2/imgproc/imgproc_c.h> //CV_AAの解決
...
void plot_image_with_cuboid_edges(cv::Mat &plot_img, const MatrixXi &box_corners_2d, const MatrixXi &edge_markers)
{
    ...
    //cv::putText(plot_img, "test", cv::Point(box_corners_2d(0, edge_markers(0,0)), box_corners_2d(1, edge_markers(0,0))), cv::FONT_HERSHEY_SIMPLEX, 1.0, cv::Scalar(255,0,0), 2, cv::LINE_AA);
}
                              ...
                              double weight_vp_angle, bool whether_normalize)
{
    ...
    if (raw_data_size > 4)
    {
        ...
        {
            printf("don't need to consider angle!\n");
            final_keep_inds = dist_keep_inds;
        }
    }
    else
    {
        // printf("raw_data_size < 4....\n");
...
void change_2d_corner_to_3d_object(const MatrixXd &box_corners_2d_float, const Vector3d &configs, const Vector4d &ground_plane_sensor,
                                   const Matrix4d &transToWolrd, const Matrix3d &invK, Eigen::Matrix<double, 3, 4> &projectionMatrix, const double box_depth_data,
                                   cuboid &sample_obj)
{
    ...
    //1.26new znocheck
    double mean_obj_z = box_depth_data;
    ...
    //1.26new depth iretemiru
    // sample_obj.pos = Vector3d(mean_obj_x, mean_obj_y, mean_obj_z);
