...
//①csvファイルでデータ取るとき////////////////////////
// void writeObjectDataToCSV(const std::string& filename, int object_id, int width, int height, double depth) {
//     std::ofstream csvFile(filename, std::ios::app);  // Open file in append mode
...
//     csvFile << object_id << "," << width << "," << height << "," << depth << "\n";
									 ...
									 MatrixXd all_lines_raw, std::vector<ObjectSet> &all_object_cuboids)
{
	...
	if (rgb_img.channels() == 3)
		cv::cvtColor(rgb_img, gray_img, cv::COLOR_BGR2GRAY); //CV_BGR2GRAYをcv::COLOR_BGR2GRAYに
	else
		gray_img = rgb_img;
	...
	for (int object_id = 0; object_id < num_2d_objs; object_id++)
	{
		...
		//10.24new
		int classname_raw = obj_bbox_coors(object_id, 5);
		double box_depth_data = obj_bbox_coors(object_id, 6);
		double box_id_data = obj_bbox_coors(object_id, 7);

		//②バウンディングボックスの幅、高さ、奥行きを表示////////
		// std::cout << "Object ID: " << object_id << std::endl;
		// std::cout << "Width: " << obj_width_raw << std::endl;
		// std::cout << "Height: " << obj_height_raw << std::endl;
		// std::cout << "Depth: " << box_depth_data << std::endl;
		//③バウンディングボックスの幅、高さ、奥行きをCSVファイルに書き込む
		// writeObjectDataToCSV("output1.csv", object_id, obj_width_raw, obj_height_raw, box_depth_data);
//ここまで堀江//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		for (int sample_down_expan_id = 0; sample_down_expan_id < down_expand_sample_all.size(); sample_down_expan_id++)
		{
			...
			// std::cout << "down_expand_sample : " << down_expand_sample << std::endl;
			...
			// int top_sample_resolution = round(min(20, obj_width_raw / 10)); //  20 pixels  原文
			int top_sample_resolution = round(min(5, obj_width_raw / 10)); //2.3new サンプリング増やした
			// std::cout << "top_sample_resolution : " << top_sample_resolution << std::endl;
			...
			Vector2d expan_distmap_lefttop = Vector2d(left_x_expan_distmap, top_y_expan_distmap); //原文
			// Vector2d expan_distmap_lefttop = Vector2d(left_x_expan_distmap + 5, top_y_expan_distmap + 10); //2.4new box範囲増やした
			Vector2d expan_distmap_rightbottom = Vector2d(right_x_expan_distmap, down_y_expan_distmap); //原文
			// Vector2d expan_distmap_rightbottom = Vector2d(right_x_expan_distmap + 5, down_y_expan_distmap + 20); //2.4new box範囲増やした
			...
			// cv::imshow("im_canny", im_canny); //2.5new cannyの調整
			cv::Mat dist_map;
			cv::distanceTransform(255 - im_canny, dist_map, cv::DIST_L2, 3); // dist_map is float datatype //CV_DIST_L2をcv::.DIST_L2に
			...
			for (int cam_roll_id = 0; cam_roll_id < cam_roll_samples.size(); cam_roll_id++)
				for (int cam_pitch_id = 0; cam_pitch_id < cam_pitch_samples.size(); cam_pitch_id++)
					for (int obj_yaw_id = 0; obj_yaw_id < obj_yaw_samples.size(); obj_yaw_id++)
					{
						...
						// std::cout <<  "sample_top_pts.cols() : " << sample_top_pts.cols() << std::endl; //2.2new forが回っているかチェック
						...
						for (int sample_top_pt_id = 0; sample_top_pt_id < sample_top_pts.cols(); sample_top_pt_id++)
						{
							...
							for (int config_id = 1; config_id < 3; config_id++) // configuration one or two of matlab version
							{
								...
								// printf("continue 4 nasi!"); //2.2new continueのチェック ここまでは結構いけてる
								if (!check_inside_box(corner_7_down, expan_distmap_lefttop, expan_distmap_rightbottom))
								{ // might be slightly different from matlab
									config_good = false;
									//2.2new boxチェック
									// std::cout << "pt7 : " << corner_7_down(0) << " , " << corner_7_down(1) << std::endl;
									// std::cout << "box_left_top : " << expan_distmap_lefttop(0) << " , " << expan_distmap_lefttop(1) << std::endl;
									// std::cout << "box_right_bottom : " << expan_distmap_rightbottom(0) << " , " << expan_distmap_rightbottom(1) << std::endl;

									if (print_details)
										printf("Configuration %d fails at corner 7, outside box\n", config_id);
									continue;
								}
								// printf("continue 5 nasi!"); //2.2new continueのチェック これが通りづらい
								...
								// printf("continue 6 nasi!"); //2.2new continueのチェック これは割と通る
								if (!check_inside_box(corner_8_down, expan_distmap_lefttop, expan_distmap_rightbottom))
								{
									config_good = false;
									// std::cout << "pt8 : " << corner_8_down(0) << " , " << corner_8_down(1) << std::endl;
									// std::cout << "box_left_top : " << expan_distmap_lefttop(0) << " , " << expan_distmap_lefttop(1) << std::endl;
									// std::cout << "box_right_bottom : " << expan_distmap_rightbottom(0) << " , " << expan_distmap_rightbottom(1) << std::endl;
									if (print_details)
								...
								// printf("continue 7 nasi!"); //2.2new continueのチェック これも通りづらい
								...
								// printf("continue  subete nasi\n"); //2.2new continueのチェック
								...
								// std::cout << "入れた値 : " << all_configs_error_one_objH.row(valid_config_number_one_objH).col(4) << std::endl; //2.2new error　値あるか確認
			...
			// std::cout << "all_configs_error_one_objHの4 : " << all_configs_error_one_objH.col(4).head(valid_config_number_one_objH) << std::endl;
			// std::cout << "all_configs_error_one_objHの4列目の行数 : " << all_configs_error_one_objH.col(4).head(valid_config_number_one_objH).rows() << std::endl; //2.2new good_proposal_idsのチェック
			...
			// std::cout << "good_proposal_ids : " <<  good_proposal_ids.size() << std::endl; //12.14debug
			...
			for (int box_id = 0; box_id < good_proposal_ids.size(); box_id++)
			{
				...
				change_2d_corner_to_3d_object(all_box_corners_2d_one_objH.block(2 * raw_cube_ind, 0, 2, 8), all_configs_error_one_objH.row(raw_cube_ind).head<3>(),
											  ground_plane_sensor, cam_pose.transToWolrd, cam_pose.invK, cam_pose.projectionMatrix, box_depth_data, *sample_obj);
						//   sample_obj->print_cuboid(); //1.15 物体の詳細表示
				...
				//10.24new
				sample_obj->classname = classname_raw;
				sample_obj->idname = box_id_data;
		...
		// std::cout << "actual_cuboid_num_small : " << actual_cuboid_num_small << std::endl; //2.2new forのチェック
		...
		for (int ii = 0; ii < actual_cuboid_num_small; ii++) // use sorted index
		{
			...
			// printf("push back!!!!!!"); //12.14debug
		}

		//堀江12.28ここをコメント外す/////////
		// std::cout << all_object_cuboids[object_id][0] << std::endl;
		//////////////////////////////////////////堀江コメント↓時間図る
		// ca::Profiler::tictoc("One 3D object total time");
